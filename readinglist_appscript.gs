...
// Google Apps Script — academic website template
// =============================================================
...
// Replace these with tags relevant to your research area.
const DEFAULT_TAGS = [
  'Systematic review', 'Meta-analysis', 'Scoping review',
  'RCT', 'Qualitative', 'Methods',
  'Conceptual', 'Review', 'Foundational',
  'Intervention', 'Measurement',
  'Mental health', 'Developmental',
  'AI', 'Digital'
];

// Keywords used to auto-suggest tags when a paper is added.
// Add, remove, or replace entries to match your field.
const TAG_KEYWORDS = {
  ...
  'Methods':           ['risk of bias', 'synthesis', 'narrative synthesis'],
  'Conceptual':        ['conceptual', 'theoretical', 'framework', 'taxonomy'],
  'Review':            ['review', 'synthesis', 'overview'],
  'Foundational':      ['foundational', 'historical', 'classic'],
  'Intervention':      ['intervention', 'treatment', 'program', 'programme', 'therapy'],
  'Measurement':       ['assessment', 'scale', 'instrument', 'measure'],
  ...
  'Digital':           ['online', 'digital', 'app', 'internet', 'telehealth'],
};
...
function searchByTitle_(title) {
  try {
    const res = UrlFetchApp.fetch(
      `https://api.crossref.org/works?query.bibliographic=${encodeURIComponent(title)}&rows=1&mailto=youremail@institution.edu`,
      { muteHttpExceptions: true });
    if (res.getResponseCode() !== 200) return null;
...
function fetchMetaFromCrossRef_(doi) {
  ...
  try {
    const res = UrlFetchApp.fetch(
      `https://api.crossref.org/works/${encodeURIComponent(doi)}?mailto=youremail@institution.edu`,
      { muteHttpExceptions: true });
